...
{
    public partial class classNotice : Rc.Cloud.Web.Common.FInitData
    {
        ...
        public static string GetClassNotice(string UserGroup_Id, int PageIndex, int PageSize)
        {
            try
            {
                ...
                dt = new BLL_UserGroup_Member().GetListByPage(strWhere, "User_ApplicationTime desc", ((PageIndex - 1) * PageSize + 1), (PageIndex * PageSize)).Tables[0];
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    listReturn.Add(new
                    {
                        ...
                        UserName = pfunction.GetUserNameByUserId(dt.Rows[i]["User_ID"].ToString()),
        ...
        public static string AuditApplyData(string UserGroup_Member_Id, string AType)
        {
            try
            {
                ...
                for (int i = 0; i < strArrMember.Length; i++)
                {
                    ...
                    modelMsg.MsgTitle = string.Format("申请加入班{0}{1}", model.UserGroup_Id, strTips);
                    modelMsg.MsgContent = string.Format("申请加入班{0}{1}", model.UserGroup_Id, strTips);
