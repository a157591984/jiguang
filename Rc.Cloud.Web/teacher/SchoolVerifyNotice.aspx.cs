...
{
    public partial class SchoolVerifyNotice : Rc.Cloud.Web.Common.FInitData
    {
        ...
        public static string GetClassNotice(string UserGroup_Id, int PageIndex, int PageSize)
        {
            try
            {
                Model_F_User FloginUser = HttpContext.Current.Session["FLoginUser"] as Model_F_User;
                ...
                dt = new BLL_UserGroup_Member().GetSchoolMemberListByPageEX(strWhere, "User_ApplicationTime desc", ((PageIndex - 1) * PageSize + 1), (PageIndex * PageSize)).Tables[0];
        ...
        public static string AuditApplyData(string UserGroup_Member_Id, string AType)
        {
            try
            {
                ...
                for (int i = 0; i < strArrMember.Length; i++)
                {
                    ...
                    modelMsg.MsgTitle = string.Format("申请加入学校{0}{1}", model.UserGroup_Id, strTips);
                    modelMsg.MsgContent = string.Format("申请加入学校{0}{1}", model.UserGroup_Id, strTips);
                    modelMsg.MsgStatus = MsgStatus.Unread.ToString();
                    modelMsg.MsgSender = userId;
                    modelMsg.MsgAccepter = model.User_ID;
                    if (model.MembershipEnum == MembershipEnum.grade.ToString())
                    {
