<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/teacher.Master" AutoEventWireup="true" CodeBehind="allTeachingPlan.aspx.cs" Inherits="Rc.Cloud.Web.teacher.allTeachingPlan" %>
        ...
        <div class="container pt">
            <div class="filter">
                ...
                <div class="filter_section">
                    ...
                    <div class="filter_row clearfix">
                        <span class="row_name">书本类型：</span>
                        <div class="row_item">
                            <ul ajax-name="resource_type">
                                <li><a href='##' class='active' ajax-value='0'>全部</a></li>
                                <li><a href='##' class='' ajax-value='1'>教案</a></li>
                                <li><a href='##' class='' ajax-value='2'>习题集</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                {#foreach $T.list as record}
                    <div class="col-xs-2">
                        ...
                        <div class="thumbnail no_purchase">
                            <a href="/teacher/TeachingplanShow.aspx?ResourceFolder_ID={$T.record.ResourceFolder_ID}" title="{$T.record.Book_Name}" target="_blank">
                        ...
                        <div class="thumbnail purchase">
                            <a href="/teacher/TeachingplanShow.aspx?ResourceFolder_ID={$T.record.ResourceFolder_ID}"  target="_blank" title="{$T.record.Book_Name}">
                                <div class="thumb"><img src="{$T.record.BookImg_Url}" {#if $T.record.imgHeight !=0}height="{$T.record.imgHeight}"{#/if}  {#if $T.record.imgWidth !=0}width="{$T.record.imgWidth}"{#/if} onerror="this.src='../images/re_nopic.jpg'"></div>
                                <div class="caption">
                                    <p class="res_name">{$T.record.Book_Name}</p>
                                    <p>￥{$T.record.BookPrice}</p>
    ...
    <script type="text/javascript">
        ...
        var loadData = function () {
            var dto = {
                ...
                Resource_Type: $('[ajax-name="resource_type"]').find('a.active').attr('ajax-value'),
                PageIndex: pageIndex,
                PageSize: ($("[data-name='pagination_select']").val() == undefined ? 12 : $("[data-name='pagination_select']").val()),
                x: Math.random()
            };
            $.ajaxWebService("allTeachingPlan.aspx/GetResourceFolderListPage", JSON.stringify(dto), function (data) {
        ...
        var LoadCity = function (pid) {
            $.ajaxWebService("allTeachingPlan.aspx/GetAreaCityInfo", "{pid:'" + pid + "',x:'" + Math.random() + "'}", function (data) { $("#ddlCity").html(data.d); }, function () { $("#ddlCity").html("<option value=\"-1\">市区</option>"); }, false);
        }
    </script>
</asp:Content>
