<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FileSyncPlan.aspx.cs" Inherits="Rc.Cloud.Web.ReCloudMgr.FileSyncPlan" %>
    ...
    <form id="form1" runat="server">
        <div class="form-inline search_bar mb">
            ...
            <input type="text" id="txtBOOK" clientidmode="Static" runat="server" autocomplete="off" placeholder="书名" class="form-control input-sm"
                ...
                pautocompleteajaxkey="BOOK_Class"
                pautocompletevectors="AutoCompleteVectors"
                pautocompleteisjp="0"
                pautocompletepagesize="10" style="width: 300px;" />
            <label>
                <input type="checkbox" id="chbCover" runat="server" />
                覆盖同步（已同步的文件有更新时选中此选项）
            </label>
            <asp:Button ID="btn" runat="server" CssClass="btn btn-primary btn-sm" Text="执行教案同步" OnClick="btn_Click" />
            <input type="button" class="btn btn-primary btn-sm" value="同步预览" onclick="javascript: window.open('FileSyncViewList.aspx?FileSyncExecRecord_Type=同步教案');" />
        ...
        <script type="text/javascript">
            var loadData = function () {
                ...
                $.ajaxWebService("FileSyncPlan.aspx/GetDataList", JSON.stringify(dto), function (data) {
                    ...
                    if ($.trim($("#hidtxtBOOK").val()) == "") {
                        if ($.trim($("#txtSDate").val()) == "") {
                            ...
                            layer.ready(function () {
                                layer.msg("请选择日期", { icon: 4, time: 1000 }, function () {
                                    $("#txtSDate").focus();
                                });
