<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/sys.Master" AutoEventWireup="true" CodeBehind="FileSyncNew.aspx.cs" Inherits="Rc.Cloud.Web.ReCloudMgr.FileSyncNew" %>
                ...
                <div class="form-inline search_bar">
                    ...
                    <div class="input-group">
                        ...
                        <div class="input-group-btn">
                            <button type="button" class="btn btn-primary btn-sm" onclick="showIframe('4')">同步学校教案</button>
                        </div>
                    </div>
                    <div class="input-group">
                        <input type="hidden" id="hidtxtSchool2" runat="server" clientidmode="Static" />
                        <input type="text" id="txtSchool2" clientidmode="Static" runat="server" autocomplete="off" class="form-control input-sm" placeholder="学校名称"
                            pautocomplete="True"
                            pautocompleteajax="AjaxAutoCompletePaged"
                            pautocompleteajaxkey="SCHOOL"
                            pautocompletevectors="AutoCompleteVectors"
                            ...
                            pautocompletepagesize="10" />
                        <div class="input-group-btn">
                            <button type="button" class="btn btn-primary btn-sm" onclick="showIframe('5')">同步学校作业</button>
                        </div>
                    </div>
                </div>
                <h3 id="iframeTitle">同步教案文件（教案以及预览图片）</h3>
                <iframe id='iframDrugFiled' frameborder='0' width='100%' height="550" src='<%=strFileSyncUrlPlan%>'></iframe>
            </div>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        function showIframe(type) {
            ...
            if (type == 1) {
                ...
                var schoolip_local = "";
                $.ajaxWebService("FileSyncNew.aspx/GetSchoolUrl", JSON.stringify(dto), function (data) {
                    var json = $.parseJSON(data.d);
                    if (json.err == "null") {
                        page = json.SchoolIP + "<%=strFileSyncUrlPlanSchool%>" + "&SchoolId=" + $.trim($("#hidtxtSchool").val());

                        schoolip_local = json.SchoolIP_Local;

                        $('#iframeTitle').html('同步学校教案');
                    }
                    else {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == "ok") {
                            page = schoolip_local + "<%=strFileSyncUrlPlanSchool%>" + "&SchoolId=" + $.trim($("#hidtxtSchool").val());
                        }
                        layer.close(idx);
                    },
                    error: function () {
                ...
                if ($.trim($("#hidtxtSchool2").val()) == "") {
                    layer.msg("请选择学校", { icon: 2, time: 1000 });
                    return false;
                }
                var dto = {
                    schoolId: $.trim($("#hidtxtSchool2").val()),
                    x: Math.random()
                };
                var schoolip_local = "";
                $.ajaxWebService("FileSyncNew.aspx/GetSchoolUrl", JSON.stringify(dto), function (data) {
                    var json = $.parseJSON(data.d);
                    if (json.err == "null") {
                        page = json.SchoolIP + "<%=strFileSyncUrlTestPaperSchool%>" + "&SchoolId=" + $.trim($("#hidtxtSchool2").val());
                        schoolip_local = json.SchoolIP_Local;

                        $('#iframeTitle').html('同步学校作业');
                    }
                    else {
                        layer.msg(json.err, { icon: 2, time: 1000 });
                        return false;
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == "ok") {
                            page = schoolip_local + "<%=strFileSyncUrlTestPaperSchool%>" + "&SchoolId=" + $.trim($("#hidtxtSchool2").val());
                        }
                        layer.close(idx);
                    },
                    error: function () {
