...
{
    public partial class ResEmpower : Rc.Cloud.Web.Common.InitPage
    {
...
where 1=1" + strWhere + " ) t where row between " + ((PageIndex - 1) * PageSize + 1) + " and " + (PageIndex * PageSize) + "  ";
        ...
        private static string GetBookAttr(DataTable dtAttr, string AttrEnum, string ResourceFolder_Id)
        {
            string temp = "0";
            try
            {
                DataRow[] drAttr = dtAttr.Select("AttrEnum='" + AttrEnum + "' and ResourceFolder_Id='" + ResourceFolder_Id + "' ");
                if (drAttr.Length > 0)
                {
                    temp = drAttr[0]["AttrValue"].ToString();
                }
                else
                {
                    // 默认是允许复制
                    if (AttrEnum == BookAttrEnum.Copy.ToString())
                    {
                        temp = "1";
                    }
                }
