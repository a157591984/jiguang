...
{
    public partial class StatsMember : Rc.Cloud.Web.Common.InitPage
    {
        ...
        [WebMethod]
        public static string GetData(string MemberType, string DateType, string SchoolId, string datetime)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(SchoolId) && SchoolId != "-1")
                {
                    ...
                    List<Model_StatsSchoolMember> StatsSchoolMemberList = new List<Model_StatsSchoolMember>();
                    BLL_StatsSchoolMember StatsSchoolMemberBll = new BLL_StatsSchoolMember();
                    StatsSchoolMemberList = StatsSchoolMemberBll.GetModelList(strWhere + " order by DateData");
                    foreach (var item in StatsSchoolMemberList)
                    {
                        strDate += item.DateData.ToString() + ",";
                        strCount += item.MemberCount.ToString() + ",";
                        strCount_N += item.NMemberCount.ToString() + ",";
                    }
                }
                else
                {
                    List<Model_StatsAllMember> StatsAllMemberList = new List<Model_StatsAllMember>();
                    BLL_StatsAllMember StatsAllMemberBll = new BLL_StatsAllMember();
                    StatsAllMemberList = StatsAllMemberBll.GetModelList(strWhere + " order by DateData");
                    foreach (var item in StatsAllMemberList)
                    {
                        strDate += item.DateData.ToString() + ",";
                        strCount += item.MemberCount.ToString() + ",";
                        strCount_N += item.NMemberCount.ToString() + ",";
