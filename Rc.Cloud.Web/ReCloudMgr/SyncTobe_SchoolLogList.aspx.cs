...
{
    public partial class SyncTobe_SchoolLogList : System.Web.UI.Page
    {
        ...
        public static string GetDataList(string strFileSyncExecRecord_Type, string strFileSyncExecRecord_Status, int PageSize, int PageIndex)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(strFileSyncExecRecord_Type))//同步学校资源84559
                {
                    strWhere += " and f.FileSyncExecRecord_Type='同步学校资源" + strFileSyncExecRecord_Type.Filter() + "' ";
                }
                else
                {
                    strWhere += " and f.FileSyncExecRecord_Type like '同步学校资源%'";
                }
 ...
 left join SysUser u on f.createUser=u.SysUser_ID where 1=1 "
                ...
                dtRes = Rc.Common.DBUtility.DbHelperSQL_Operate.Query(strSql).Tables[0];

                int rCount = Convert.ToInt32(Rc.Common.DBUtility.DbHelperSQL_Operate.GetSingle(strSqlCount).ToString());
                int inum = 0;
