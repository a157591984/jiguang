<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SyncPlanFile_Tobe.aspx.cs" Inherits="Rc.Cloud.Web.ReCloudMgr.SyncPlanFile_Tobe" %>
...
<body class="bg_white">
    <form id="form1" runat="server">
        <div class="form-inline search_bar mb">
            <input runat="server" type="button" id="btnConfirm" value="执行教案同步" class="btn btn-primary btn-sm" />
            <asp:Button ID="btnPlan" runat="server" CssClass="btn btn-primary btn-sm hide" Text="执行教案同步" OnClick="btnPlan_Click" />
            <input type="button" class="btn btn-primary btn-sm" value="同步预览" onclick="javascript: window.open('FileSyncViewList.aspx?FileSyncExecRecord_Type=同步教案new');" />
            <input type="button" value="执行记录" class="btn btn-primary btn-sm" onclick="javascript: window.open('SyncTobe_LogList.aspx?FileSyncExecRecord_Type=同步教案new');" />
            ...
            {#foreach $T.list as record}
                <tr>
                    ...
                    <td class="opera">
                        ...
                        <a href="SyncPlanFile_Tobe.aspx?bookId={$T.record.bookId}&SysUser_ID=<%=SysUser_ID %>">资源明细</a>
                        {#/if}
                        {#if $T.record.IsCanExecing=='1'}
                        <a href="javascript:;" data-name="sync_plan" data-bookid="{$T.record.bookId}" data-rtrfid="{$T.record.rtrfId}">执行同步</a>
                        {#/if}
        ...
        <script type="text/javascript">
            var loadData = function () {
                ...
                $.ajaxWebService("SyncPlanFile_Tobe.aspx/GetDataList", JSON.stringify(dto), function (data) {
            ...
            $(function () {
                ...
                $(document).on('click', '[data-name="sync_plan"]', function () {
                ...
                $("#btnConfirm").click(function () {
                    layer.confirm("您确定要执行同步教案吗？", { icon: 2, title: "提示" }, function () {
                        $("#btnPlan").click();
                    });
                });
