<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/sys.Master" AutoEventWireup="true" CodeBehind="NoticeBoardList.aspx.cs" Inherits="Rc.Cloud.Web.ReCloudMgr.NoticeBoardList" %>
        ...
        <div class="pa">
            ...
            <div class="panel">
                <div class="panel-body">
                    ...
                    <tr>
                        <td>{$T.record.notice_title}</td>
                        <td>{$T.record.start_time}</td>
                        <td>{$T.record.end_time}</td>
                        <td>{$T.record.create_time}</td>
                        <td class="opera">
                            <a href="javascript:edit('{$T.record.notice_id}');">编辑</a>
                            <a href="javascript:del('{$T.record.notice_id}');">删除</a>
                        </td>
                    </tr>
                    {#/for}
                    </textarea>
...
<asp:Content ID="Content3" ContentPlaceHolderID="JSContent" runat="server">
    <script type="text/javascript">
        ...
        var loadData = function () {
            ...
            $.ajaxWebService("NoticeBoardList.aspx/GetList", JSON.stringify(dto), function (data) {
        ...
        function edit(notice_id) {
            layer.open({
                ...
                title: '编辑',
                area: ['850px', '640px'],
                content: 'NoticeBoardEdit.aspx?notice_id=' + notice_id
            });
        }
        function Add() {
            layer.open({
                ...
                title: '新增',
                area: ['850px', '640px'],
                content: 'NoticeBoardEdit.aspx'
            });
        }
        var del = function (notice_id) {
            var index = layer.confirm('删除数据？', { icon: 3, title: '提示' }, function () {
                layer.close(index);//关闭确认弹窗
                $.ajaxWebService("NoticeBoardList.aspx/delete", "{notice_id:'" + notice_id + "',x:'" + Math.random() + "'}", function (data) {
                    if (data.d == "1") {
