...
{
    public partial class FileSync : Rc.Cloud.Web.Common.InitPage
    {
        ...
        public void ResSynchroTeachingPlan(string strDate)
        {
            ...
            strSql = string.Format(@"select  ParticularYear,GradeTerm,Resource_Version,Subject,Resource_Type,ResourceToResourceFolder_id  as FileName 
                ...
                and ba.AuditState='1'
                order by t1.Createtime desc", Resource_TypeConst.ScienceWord类型文件, Resource_TypeConst.class类型文件, Resource_TypeConst.class类型微课件, strDate);
...
and convert(varchar(10),ba.CreateTime,120)='{3}'
and ba.AuditState='1'
order by t2.Createtime desc", Resource_TypeConst.ScienceWord类型文件, Resource_TypeConst.class类型文件, Resource_TypeConst.class类型微课件, strDate);
            DataTable dt = new DataTable();
