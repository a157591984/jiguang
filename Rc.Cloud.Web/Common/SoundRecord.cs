...
namespace Rc.Cloud.Web.Common
{
    public class SoundRecord
    {
        ...
        {
            // 初始化音频捕捉设备 
            AddLogInfo(DateTime.Now + "写入数据：初始化音频捕捉设备");
            InitCaptureDevice();
            // 设定录音格式 
            AddLogInfo(DateTime.Now + "写入数据：设定录音格式");
        ...
        {
            // 创建录音文件 
            AddLogInfo(DateTime.Now + "写入数据：创建录音文件");
            CreateSoundFile();
            // 创建一个录音缓冲区，并开始录音
            AddLogInfo(DateTime.Now + "写入数据：创建一个录音缓冲区，并开始录音");
            CreateCaptureBuffer();
            // 建立通知消息,当缓冲区满的时候处理方法 
            AddLogInfo(DateTime.Now + "写入数据：建立通知消息,当缓冲区满的时候处理方法 ");
            InitNotifications();
        ...
        {
            // 关闭通知消息 
            AddLogInfo(DateTime.Now + "写入数据：关闭通知消息 ");
            if (null != mNotificationEvent)
                mNotificationEvent.Set();
            // 停止录音 
            AddLogInfo(DateTime.Now + "写入数据：停止录音 ");
            mRecBuffer.Stop();
            // 写入缓冲区最后的数据 
            AddLogInfo(DateTime.Now + "写入数据：写入缓冲区最后的数据 ");
            RecordCapturedData();
            // 回写长度信息 
            AddLogInfo(DateTime.Now + "写入数据：回写长度信息 ");
        ...
        private WaveFormat CreateWaveFormat()
        {
            ...
            format.SamplesPerSecond = 3072;        // 16KHz 16000
        ...
        private void RecordCapturedData()
        {
            ...
            AddLogInfo(DateTime.Now + "写入数据：" + System.Text.Encoding.Default.GetString(CaptureData));
        ...
        public void AddLogInfo(string txt, bool isAppend = true)
        {
            string logPath = AppDomain.CurrentDomain.BaseDirectory + "\\Upload\\RecordingLog\\";
