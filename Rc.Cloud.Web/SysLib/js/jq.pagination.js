...
jQuery.fn.pagination = function (maxentries, opts) {
    opts = jQuery.extend({
        ...
        prev_text: "&laquo;", //“前一页”分页按钮上显示的文字
        next_text: "&raquo;", //“下一页”分页按钮上显示的文字
        ellipse_text: "...", //省略的页数用什么文字表示
        prev_show_always: true, //是否显示“前一页”分页按钮
        next_show_always: true, //是否显示“下一页”分页按钮
        page_select_show: true,//是否显示“每页显示多少条”选项
        ...
        */
        function drawLinks() {
            ...
            var appendItem = function (page_id, appendopts) {
                ...
                if (page_id == current_page && appendopts.text != opts.prev_text && appendopts.text != opts.next_text) {
                ...
                panel.find("[data-name='pagination']").append(lnk);
            }
            var pageSelect = "";
            if (opts.page_select_show) {
                ...
                pageSelect += "</select>&nbsp;&nbsp;";
                pageSelect = "，每页显示" + pageSelect + "条";
            }

            panel.append("<div class=\"pagination_info\">共" + maxentries + "条" + pageSelect + "</div>");
            //appendItem(current_page, { text: "页码:" + (current_page + 1) + "/" + np, classes: "home" });

            panel.append("<ul class=\"pagination\" data-name=\"pagination\"></ul>");
            ...
            if (interval[0] > 0 && opts.num_edge_entries > 0) {
                ...
                if (opts.num_edge_entries < interval[0] && opts.ellipse_text) {
                    jQuery("<li><a href='javascript:void(0);'>" + opts.ellipse_text + "</a></li>").appendTo(panel.find("[data-name='pagination']")).bind("click", getClickHandler(interval[0] - 1));
                }
            }
            ...
            if (interval[1] < np && opts.num_edge_entries > 0) {
                if (np - opts.num_edge_entries > interval[1] && opts.ellipse_text) {
                    jQuery("<li><a href='javascript:void(0);'>" + opts.ellipse_text + "</a></li>").appendTo(panel.find("[data-name='pagination']")).bind("click", getClickHandler(interval[1]));
                }
