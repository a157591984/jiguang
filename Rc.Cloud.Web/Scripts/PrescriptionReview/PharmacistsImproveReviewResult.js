var PharmacistsImproveReviewResult = {};

PharmacistsImproveReviewResult.Del = function (e, id) {
    if (confirm('确定要删除吗？')) {
...
PharmacistsImproveReviewResult.Save = function () {
    var id = $("#hidImproveReviewResult_ID").val();
    if (id.length > 0) {
        //修改
        PharmacistsImproveReviewResult.Edit(id);
    }
    else {
        //新增
        PharmacistsImproveReviewResult.Add();
    }
}

PharmacistsImproveReviewResult.ShowEdit = function (e, id) {
...
PharmacistsImproveReviewResult.Edit = function (id) {
    //从页面中获取值
    ...
    { "key": "UpdatePharmacistsImproveReviewResult",
        "id": id,
        ...
        "detail": detail
            ...
            function (data) {
                if (data != "0" && data != "1") {
                    ...
                    strhtml += '<td class="aui_td_content">';
                    strhtml += '<a href="javascript:void(0)" onclick=\'PharmacistsImproveReviewResult.ShowEdit(this,"' + data + '")\'>修改</a> ';
                    strhtml += ' <a href="javascript:void(0)" onclick=\'PharmacistsImproveReviewResult.Del(this,"' + data + '")\'>删除</a></td> </tr>';
                    $("#tbPharmacistsReview").append(strhtml);
...
PharmacistsImproveReviewResult.Add = function () {
    //从页面中获取值
    var prescription_ID = $("#hidPrescription_ID").val();
    var prescription_Code = $("#lbBasePrescription_Code").val();
    var bacth_ID = $("#hidBacth_ID").val();
    ...
    //保存到数据库
    $.post("/Ajax/HospitalConfigAjax.aspx",
    { "key": "InsertPharmacistsImproveReviewResult",
        "prescription_ID": prescription_ID,
        "prescription_Code": prescription_Code,
        "bacth_ID": bacth_ID,
        "drugId": drugId,
        ...
        "detail": detail
            ...
            function (data) {
                if (data != "0" && data != "1") {
                    showTips('保存成功', '', '2');
                    //成功后，拼接字符串，添加到表格中
                    $("#tbPharmacistsReview").style.display = "";
                    var strhtml = '';
                    strhtml = '<tr id="' + data + '" class="tr_con_002">';
                    strhtml += '<td class="aui_td_content">';
                    strhtml += $("#ddlImproveReviewResult_ReviewID option:selected").text();
