...
<div id="<%=id%>" class="aui_dialog_wrap <%="aui_" + skin%>">\
  ...
  <% if (border) { %>\
	<table class="aui_table">\
		<tr>\
		  <td class="aui_border aui_left_top" style="background:#000;"></td>\
		  <td class="aui_border aui_top" style="background:#000;"></td>\
		  <td class="aui_border aui_right_top" style="background:#000;"></td>\
		</tr>\
		<tr>\
		  <td class="aui_border aui_left" style="background:#000;"></td>\
		  <td class="aui_center">\
			<% } %>\
		  ...
		  <td class="aui_border aui_right" style="background:#000;"></td>\
		</tr>\
		<tr>\
		  <td class="aui_border aui_left_bottom" style="background:#000;"></td>\
		  <td class="aui_border aui_bottom" style="background:#000;"></td>\
		  <td class="aui_border aui_right_bottom" style="background:#000;"></td>\
		</tr>\
	</table>\
	<% } %>\
...
';
    ...
    var defaults = 
    {
        ...
        yesFn: null, 			// 确定按钮回调函数
        noFn: null, 				// 取消按钮回调函数
        yesText: '确定', // 确定按钮文本. 默认'确定'\u786E\u5B9A
        noText: '取消', 	// 取消按钮文本. 默认'取消'\u53D6\u6D88
        width: 'auto', 			// 宽度
        height: 'auto', 			// 高度
        skin: 'default', 		// 皮肤
        icon: null, 				// 消息图标
    ...
    $.fn.dialog.loadSkin = function (skin, bg) {
        var load = function (name) {
            ...
            //alert($.dialog.defaults.path + '/skin/' + name + '.css');
            $.getStyle("../styles/Dialog.css");
    ...
    var allReady = function () {
        ...
        // 载入核心CSS文件
        //$.dialog.defaults.path + '/core/art.dialog.css'
        if (!dialogReady) return $.getStyle("../styles/Dialog.css", function () {
            dialogReady = true;
            allReady();
        });
