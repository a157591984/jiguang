<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/sys.Master" AutoEventWireup="true" CodeBehind="KnowledgePointList.aspx.cs" Inherits="Rc.Cloud.Web.Sys.KnowledgePointList" %>
                        ...
                        {#foreach $T.list as record}
                            <tr>
                                ...
                                <td class="opera">
                                    ...
                                    {#if $T.record.IsLast=='0'}                                        
                                        <a href="##" onclick="AddSub(this,'{$T.record.S_KnowledgePoint_Id}')" >新增下级</a>
                                    {#else}
                                    <a href="##" onclick="UpdateAttr('{$T.record.S_KnowledgePoint_Id}')" >编辑扩展属性</a>
                                    {#/if}
...
<asp:Content ID="Content3" ContentPlaceHolderID="JSContent" runat="server">
    <script type="text/javascript">
            ...
            $("#btnAdd").click(function () {
                layer.ready(function () {
                    layer.open({
                        ...
                        fix: false,
                        area: ["450px", "500px"],
                        content: "KnowledgePointEdit.aspx?parentId=0&GradeTerm=" + $('[ajax-name="GradeTerm"] a[class="active"]').attr("ajax-value")
        ...
        var loadSubDict = function (hId, sId, pId, objContainer) {
            ...
            $.ajaxWebService("KnowledgePointList.aspx/GetSubDictList", JSON.stringify(dto), function (data) {
                ...
                switch (hId) {
                    ...
                    case '934A3541-116E-438C-B9BA-4176368FCD9B':
                        $('[ajax-name="Resource_Version"] a:eq(0)').click();
                        break;
                    case '74958B74-D2A4-4ACD-BB4E-F48C59329F40':
                        $('[ajax-name="Book_Type"] a:eq(0)').click();
                        break;
                }
        ...
        var loadData = function () {
            ...
            $.ajaxWebService("KnowledgePointList.aspx/GetDataList", JSON.stringify(dto), function (data) {
        ...
        var loadSubData2 = function ($toggle) {
            if ($(_objTD).html() != undefined) {
                ...
                if ($toggle == "0") {//加载子级数据
                    ...
                    $.ajaxWebService("KnowledgePointList.aspx/GetSubDataList", JSON.stringify(dto), function (data) {
                        var json = $.parseJSON(data.d);
                        if (json.err == "null") {
            ...
            layer.ready(function () {
                layer.ready(function () {
                    layer.open({
                        ...
                        fix: false,
                        area: ["450px", "500px"],
                        content: "KnowledgePointEdit.aspx?GradeTerm=" + $('[ajax-name="GradeTerm"] a[class="active"]').attr("ajax-value")
                        + "&Subject=" + $('[ajax-name="Subject"] a[class="active"]').attr("ajax-value")
                        + "&Resource_Version=" + $('[ajax-name="Resource_Version"] a[class="active"]').attr("ajax-value")
                        + "&Book_Type=" + $('[ajax-name="Book_Type"] a[class="active"]').attr("ajax-value")
                        + "&parentId=" + parentId
        ...
        function UpdateData(obj, kpId, parentId) {
            ...
            layer.ready(function () {
                layer.open({
                    ...
                    fix: false,
                    area: ["450px", "500px"],
                    content: "KnowledgePointEdit.aspx?kpId=" + kpId + "&parentId=" + parentId
                })
            });
        }
        ...
        function DeleteData(obj, kpId) {
            layer.ready(function () {
                layer.confirm("确定要删除吗？", { icon: 2, title: "删除提示" }, function () {
                    $.ajaxWebService("KnowledgePointList.aspx/DeleteData", "{S_KnowledgePoint_Id:'" + kpId + "',x:" + Math.random() + "}", function (data) {
                        if (data.d == "1") {
                            ...
                            layer.msg('此知识点已添加到试卷中，删除失败！', { icon: 2 });
                            return false;
                        }
                        else {
        ...
        //编辑属性扩展
        function UpdateAttr(S_KnowledgePoint_Id) {
            layer.ready(function () {
                layer.open({
                    type: 2,
                    title: '编辑属性扩展',
                    fix: false,
                    area: ["85%", "85%"],
                    content: "KnowledgePointAttrList.aspx?S_KnowledgePoint_Id=" + S_KnowledgePoint_Id
                })
            });
        }
    </script>
