<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DictRelationDetail_Edit.aspx.cs" Inherits="Rc.Cloud.Web.Sys.DictRelationDetail_Edit" %>
...
<body>
    ...
    <script type="text/javascript">
        ...
        var loadDataLast = function () {
            var dto = {
                Id: $('[ajax-name="FirstCode"]').find('a.active').attr('ajax-value'),
                DictRelation_Id: "<%=DictRelation_Id%>",
                x: Math.random()
            };
            $.ajaxWebService("DictRelationDetail_Edit.aspx/GetLastData", JSON.stringify(dto), function (data) {
        ...
        var AddData = function () {
            var dto = {
                FirstCode: "<%=Type%>",
                SecondCode: $('[ajax-name="FirstCode"]').find('a.active').attr('ajax-value'),
                LastCode: $("#hidLastCode").val(),
                DictRelation_Id: "<%=DictRelation_Id%>",
                x: Math.random()
            };
            $.ajaxWebService("DictRelationDetail_Edit.aspx/AddData", JSON.stringify(dto), function (data) {
                if (data.d == "1") {
