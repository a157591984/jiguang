...
namespace Rc.WebControls.Utility {
	...
	public class Chinese2Spell {
		...
		private static Regex regexHazi = new Regex("^[\u4e00-\u9fa5]$");
		...
		/// <returns></returns>
		public static bool IsHanzi(string c) {
			return regexHazi.IsMatch(c);
		}
				...
				if (regex.IsMatch(noWChar[j].ToString())) {
					...
					if (chrAsc > 0 && chrAsc < 160) {
						...
						if (chrAsc == -9254)  // 修正“圳”字
							...
							for (int i = (_pyValue.Length - 1); i >= 0; i--) {
								if (_pyValue[i] <= chrAsc) {
									pyString = pyString + splitChar + _pyName[i];
									break;
								}
							}
						}
		...
		#region 将汉字字符串转换为字母字符串，字母之间没有空格

		/// <summary>
		/// 将汉字字符串转换为字母字符串，字母之间没有空格
		/// </summary>
		...
		public static string GetSpellStringLower(string str) {
			...
			foreach (char c in str) {
				int ascInt = (int)c;
				if ((ascInt >= 48 && ascInt <= 57) || (ascInt >= 65 && ascInt <= 90) || (ascInt >= 97 && ascInt <= 122)) 			//字母和符号原样保留
					tempStr += c.ToString();
				else if (IsHanzi(c.ToString())) {		//汉字转换为字母
					ascInt = (int)GetSpellCharLower(c.ToString())[0];
					//如果转换后的字符为字母和数字，那么就添加上去
					if ((ascInt >= 48 && ascInt <= 57) || (ascInt >= 65 && ascInt <= 90) || (ascInt >= 97 && ascInt <= 122)) 
		...
		#region 将汉字字符串转换为字母字符串，返回的字母之间用空格分隔

		/// <summary>
		/// 将汉字字符串转换为字母字符串，返回的字母之间用空格分隔
		/// </summary>
		/// <param name="str">要转换的汉字字符串</param>
		/// <returns>拼音缩写</returns>
		...
		#region 将单个汉字转换为单个字幕（去拼音第一个字符）
		/// <summary>
		/// 将单个汉字转换为单个字幕（去拼音第一个字符）
		/// </summary>
